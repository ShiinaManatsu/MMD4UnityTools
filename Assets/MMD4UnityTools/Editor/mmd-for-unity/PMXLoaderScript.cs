    ...
    public partial class PMXLoaderScript
    {
        ...
        private PMXFormat.Material ReadMaterial()
        {
            PMXFormat.Material result = new PMXFormat.Material
            {
                ...
                diffuse_color = ReadSinglesToColor(binary_reader_), // dr, dg, db, da // 減衰色
                specular_color = ReadSinglesToColor(binary_reader_, 1), // sr, sg, sb // 光沢色
                specularity = binary_reader_.ReadSingle(),
                ambient_color = ReadSinglesToColor(binary_reader_, 1), // mr, mg, mb // 環境色(ambient)
                flag = (PMXFormat.Material.Flag)binary_reader_.ReadByte(),
                edge_color = ReadSinglesToColor(binary_reader_), // r, g, b, a
                edge_size = binary_reader_.ReadSingle(),
                usually_texture_index = CastIntRead(binary_reader_, format_.header.textureIndexSize),
                sphere_texture_index = CastIntRead(binary_reader_, format_.header.textureIndexSize),
                sphere_mode = (PMXFormat.Material.SphereMode)binary_reader_.ReadByte(),
                common_toon = binary_reader_.ReadByte()
            };
